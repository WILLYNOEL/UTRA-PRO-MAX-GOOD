...
class HydraulicPumpTester:
    ...
    def test_comprehensive_audit_system(self):
        ...
        print("\n🔍 Testing Comprehensive Audit System with Intelligent Field Analysis...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                missing_fields = [field for field in required_fields if field not in result]
                if missing_fields:
                    self.log_test("Audit System - Structure", False, f"Missing fields: {missing_fields}")
                    return False
                
                # Test expected overall score (40-60 due to multiple issues)
                overall_score = result.get("overall_score", 0)
                if not (40 <= overall_score <= 60):
                    self.log_test("Audit System - Overall Score", False, 
                                f"Expected score 40-60, got {overall_score}")
                    return False
                
                # Test performance comparisons
                performance_comparisons = result.get("performance_comparisons", [])
                if len(performance_comparisons) < 3:
                    self.log_test("Audit System - Performance Comparisons", False, 
                                f"Expected at least 3 comparisons, got {len(performance_comparisons)}")
                    return False
                ...
                if not flow_comparison:
                    self.log_test("Audit System - Flow Analysis", False, "Missing flow rate comparison")
                    return False
                
                flow_deviation = flow_comparison.get("deviation_from_required", 0)
                if not (-30 <= flow_deviation <= -20):  # Should be around -25%
                    self.log_test("Audit System - Flow Deviation", False, 
                                f"Expected ~-25% deviation, got {flow_deviation:.1f}%")
                    return False
                ...
                if not hmt_comparison:
                    self.log_test("Audit System - HMT Analysis", False, "Missing HMT comparison")
                    return False
                
                hmt_deviation = hmt_comparison.get("deviation_from_required", 0)
                if not (35 <= hmt_deviation <= 45):  # Should be around +40%
                    self.log_test("Audit System - HMT Deviation", False, 
                                f"Expected ~+40% deviation, got {hmt_deviation:.1f}%")
                    return False
                ...
                if not current_comparison:
                    self.log_test("Audit System - Current Analysis", False, "Missing current comparison")
                    return False
                
                current_deviation = current_comparison.get("deviation_from_required", 0)
                if not (22 <= current_deviation <= 32):  # Should be around +27%
                    self.log_test("Audit System - Current Deviation", False, 
                                f"Expected ~+27% deviation, got {current_deviation:.1f}%")
                    return False
                
                # Test diagnostics (hydraulic, electrical, mechanical)
                diagnostics = result.get("diagnostics", [])
                if len(diagnostics) < 3:
                    self.log_test("Audit System - Diagnostics", False, 
                                f"Expected at least 3 diagnostics, got {len(diagnostics)}")
                    return False
                ...
                if missing_categories:
                    self.log_test("Audit System - Diagnostic Categories", False, 
                ...
                if len(recommendations) < 2:
                    self.log_test("Audit System - Recommendations", False, 
                                f"Expected at least 2 recommendations, got {len(recommendations)}")
                    return False
                
                # Verify critical/high priority recommendations
                priorities = [rec["priority"] for rec in recommendations]
                if not any(priority in ["critical", "high"] for priority in priorities):
                    self.log_test("Audit System - Priority Recommendations", False, 
                                "No critical or high priority recommendations found")
                    return False
                
                # Test economic analysis with payback
                economic_analysis = result.get("economic_analysis", {})
                if not economic_analysis:
                    self.log_test("Audit System - Economic Analysis", False, "Missing economic analysis")
                    return False
                ...
                if missing_economic:
                    self.log_test("Audit System - Economic Fields", False, 
                ...
                if not action_plan:
                    self.log_test("Audit System - Action Plan", False, "Missing action plan")
                    return False
                
                # Verify action plan phases
                if "phases" not in action_plan:
                    self.log_test("Audit System - Action Plan Phases", False, "Missing phases in action plan")
                    return False
                
                phases = action_plan["phases"]
                if len(phases) < 2:
                    self.log_test("Audit System - Action Plan Phases", False, 
                                f"Expected at least 2 phases, got {len(phases)}")
                    return False
                
                # Test executive summary
                executive_summary = result.get("executive_summary", {})
                if not executive_summary:
                    self.log_test("Audit System - Executive Summary", False, "Missing executive summary")
                    return False
                ...
                if missing_summary:
                    self.log_test("Audit System - Executive Summary Fields", False, 
                                f"Missing summary fields: {missing_summary}")
                    return False
                
                self.log_test("Comprehensive Audit System", True, 
                            f"Overall Score: {overall_score}/100, Diagnostics: {len(diagnostics)}, "
                            f"Recommendations: {len(recommendations)}, Payback: {economic_analysis.get('payback_months', 'N/A')} months")
                return True
            else:
                self.log_test("Comprehensive Audit System", False, f"Status: {response.status_code}")
                if response.status_code >= 400:
                    try:
                        error_detail = response.json()
                        self.log_test("Audit System Error Detail", False, f"Error: {error_detail}")
                    except:
                        self.log_test("Audit System Error Detail", False, f"Response: {response.text[:200]}")
                return False
        except Exception as e:
