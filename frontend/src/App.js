...
function App() {
  ...
  // SCHÉMA SURFACE ASPIRATION - LIGNE DIRECTE (1 pompe) OU COLLECTEURS (plusieurs)
  const drawProfessionalSurfaceAspiration = (ctx, canvas) => {
    // Nettoyage complet
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    
    // PARAMÈTRES FIXES DU SCHÉMA
    const centerX = canvas.width / 2;
    const centerY = canvas.height / 2;
    const tankX = centerX - 200;
    const tankW = 100;
    const tankH = 80;
    const pumpCount = drawingData.pump_count || 1;
    
    // 1. BÂCHE D'ASPIRATION FIXE
    ctx.strokeStyle = '#5D4037';
    ctx.lineWidth = 3;
    ctx.fillStyle = '#EFEBE9';
    ctx.fillRect(tankX, centerY - tankH/2, tankW, tankH);
    ctx.strokeRect(tankX, centerY - tankH/2, tankW, tankH);
    
    // Niveau d'eau dans bâche
    ctx.fillStyle = '#2196F3';
    ctx.globalAlpha = 0.3;
    ctx.fillRect(tankX + 5, centerY - tankH/2 + 15, tankW - 10, tankH - 20);
    ctx.globalAlpha = 1.0;
    ...
    ctx.fillText('BÂCHE ASPIRATION', tankX + tankW/2, centerY - tankH/2 - 15);
    
    // 2. DÉCISION : LIGNE DIRECTE ou COLLECTEURS selon nombre de pompes
    if (pumpCount === 1) {
      ...
      for (let j = 0; j < 6; j++) {
        ...
        ctx.lineTo(pumpX + Math.cos(angle) * 12, pumpY + Math.sin(angle) * 12);
        ctx.stroke();
      }
      ...
      ctx.lineWidth = 6;
      ctx.beginPath();
      ctx.moveTo(pumpX + 20, pumpY);
      ctx.lineTo(centerX + 200, pumpY);
      ctx.stroke();
      
      // Équipements sélectionnés sur refoulement direct
      drawSelectedEquipments(ctx, pumpX + 60, 0, pumpY, 6, 35);
      
      // Tag pompe
      ctx.fillStyle = '#0D47A1';
      ctx.font = 'bold 12px Arial';
      ctx.textAlign = 'center';
      ctx.fillText('P-01', pumpX, pumpY + 40);
      ctx.font = '9px Arial';
      ctx.fillText(`${drawingData.flow_rate || 0}m³/h`, pumpX, pumpY + 52);
      ctx.fillText(`${drawingData.total_head || 0}m HMT`, pumpX, pumpY + 64);
      
      // Annotations lignes directes
      ctx.fillStyle = '#FF9800';
      ctx.font = '8px Arial';
      ctx.textAlign = 'center';
      ctx.fillText(`DN${drawingData.suction_diameter || 100}`, tankX + tankW + 50, centerY - 15);
      ctx.fillText('ASPIRATION', tankX + tankW + 50, centerY - 5);
      
      ctx.fillStyle = '#4CAF50';
      ctx.fillText(`DN${drawingData.discharge_diameter || 80}`, pumpX + 100, pumpY - 15);
      ctx.fillText('REFOULEMENT', pumpX + 100, pumpY - 5);
      ctx.fillText(`${drawingData.flow_rate || 0}m³/h`, pumpX + 100, pumpY + 15);
      
    } else {
      // ===== COLLECTEURS POUR PLUSIEURS POMPES =====
      const pumpSpacing = 80;
      const startY = centerY - ((pumpCount - 1) * pumpSpacing) / 2;
      
      // COLLECTEUR D'ASPIRATION VERTICAL
      const collecteurAspirationX = centerX - 100;
      const collecteurStartY = startY - 40;
      const collecteurEndY = startY + ((pumpCount - 1) * pumpSpacing) + 40;
      ...
      // Tuyauterie bâche vers collecteur
      ctx.beginPath();
      ctx.moveTo(tankX + tankW, centerY);
      ...
      for (let i = 0; i < pumpCount; i++) {
        ...
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.moveTo(collecteurAspirationX, pumpY);
        ctx.lineTo(pumpX - 25, pumpY);
        ctx.stroke();
        ...
        ctx.font = '8px Arial';
        ctx.fillText(isStandby ? 'SECOURS' : 'SERVICE', pumpX, pumpY + 47);
      }
      
      // COLLECTEUR REFOULEMENT
      const collecteurRefoulementX = centerX + 80;
      ...
      for (let i = 0; i < pumpCount; i++) {
        ...
        ctx.lineWidth = 5;
        ctx.beginPath();
        ctx.moveTo(pumpX + 25, pumpY);
        ctx.lineTo(collecteurRefoulementX, pumpY);
        ctx.stroke();
      }
      ...
      drawSelectedEquipments(ctx, collecteurRefoulementX + 40, 0, centerY, 6, 35);
      
      // Annotations collecteurs
      ctx.fillStyle = '#FF9800';
      ctx.font = '9px Arial';
      ctx.textAlign = 'center';
      ctx.save();
    ...
    // 3. DONNÉES TECHNIQUES DYNAMIQUES (bas de page)
    const dataX = 50;
    const dataY = centerY + 150;
    ...
    const techData = [
      `TYPE: Surface avec aspiration`,
      `POMPES: ${pumpCount} × ${drawingData.pumps_in_service || pumpCount} service`,
      `DÉBIT ${pumpCount === 1 ? '' : 'UNITAIRE'}: ${drawingData.flow_rate || 0} m³/h`,
      ...(pumpCount > 1 ? [`DÉBIT TOTAL: ${(drawingData.flow_rate || 0) * pumpCount} m³/h`] : []),
      `HMT: ${drawingData.total_head || 0} m`,
      `DN ASPIRATION: ${drawingData.suction_diameter || 100} mm`,
    ...
    drawEquipmentLegend(ctx, canvas, pumpCount === 1 ? 'SURFACE (1 POMPE)' : 'SURFACE (COLLECTEURS)');
  };
  ...
  // SCHÉMA FORAGE FIXE - DIMENSIONS STATIQUES (comme référence utilisateur)
  const drawProfessionalForage = (ctx, canvas) => {
    // Nettoyage complet
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    
    // PARAMÈTRES FIXES DU SCHÉMA (indépendants des valeurs saisies)
    const centerX = canvas.width / 2;
    const groundLevel = 200; // Position fixe du sol
    const wellX = centerX - 100;
    const wellWidth = 40;
    const wellDepth = 300; // Profondeur forage FIXE
    const towerX = centerX + 200;
    const towerHeight = 150; // Hauteur château FIXE
    const reservoirH = 50; // Hauteur réservoir FIXE
    
    // 1. NIVEAU SOL (ligne technique fixe)
    ...
    ctx.setLineDash([8, 4]);
    ctx.beginPath();
    ctx.moveTo(wellX - wellWidth/2, waterLevelY);
    ctx.lineTo(wellX + wellWidth/2, waterLevelY);
    ctx.stroke();
    ctx.setLineDash([]);
    
    // Eau dans le forage (en dessous du niveau d'eau)
    ctx.fillStyle = '#3498DB';
    ctx.globalAlpha = 0.3;
    ctx.fillRect(wellX - wellWidth/2 + 2, waterLevelY, wellWidth - 4, wellDepth - waterLevelFromSurface);
    ctx.globalAlpha = 1.0;
    
    // 4. POMPE SUBMERSIBLE FIXE (à 80% de la profondeur)
    const pumpY = groundLevel + wellDepth * 0.8;
    ...
    // Flèches débit (fixes)
    const arrowPositions = [pumpY - 60, pumpY - 120, pumpY - 180];
    for (let arrowY of arrowPositions) {
      if (arrowY > groundLevel + 30) {
        ctx.strokeStyle = '#27AE60';
        ctx.lineWidth = 2;
        ctx.beginPath();
    ...
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(centerX + 50, pipeY - 5);
    ctx.lineTo(centerX + 65, pipeY);
    ...
    // ÉQUIPEMENTS SÉLECTIONNÉS - Utilisation de la fonction générique
    drawSelectedEquipments(ctx, wellX + 80, 0, pipeY, pipeWidth, 35);
    ...
    ctx.lineTo(towerX + towerW/2 - 15, towerBaseY + reservoirH);
    ctx.moveTo(towerX + towerW/2 + 15, groundLevel);
    ctx.lineTo(towerX + towerW/2 + 15, towerBaseY + reservoirH);
    ctx.stroke();
    ...
    ctx.lineWidth = 2;
    const braceY1 = groundLevel - supportHeight/3;
    const braceY2 = groundLevel - supportHeight*2/3;
    for (let braceY of [braceY1, braceY2]) {
      ctx.beginPath();
      ctx.moveTo(towerX + towerW/2 - 15, braceY);
      ctx.lineTo(towerX + towerW/2 + 15, braceY);
      ctx.stroke();
    }
    ...
    ctx.fillRect(towerX, towerBaseY, towerW, reservoirH);
    ctx.strokeRect(towerX, towerBaseY, towerW, reservoirH);
    
    // Niveau liquide fixe
    const liquidLevel = 0.7;
    ctx.fillStyle = '#9B59B6';
    ctx.fillRect(towerX + 5, towerBaseY + reservoirH * (1 - liquidLevel), towerW - 10, reservoirH * liquidLevel);
    ...
    ctx.lineTo(wellX - 60, waterLevelY);
    ctx.lineTo(wellX - 40, waterLevelY);
    ctx.stroke();
    ctx.setLineDash([]);
    
    // Flèches niveau dynamique
    ctx.strokeStyle = '#2196F3';
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(wellX - 55, groundLevel);
    ctx.lineTo(wellX - 55, groundLevel + 5);
    ctx.moveTo(wellX - 55, waterLevelY);
    ...
    ctx.lineTo(towerX + towerW + 10, towerBaseY);
    ctx.stroke();
    ctx.setLineDash([]);
    
    // Flèches hauteur château
    ctx.strokeStyle = '#E91E63';
    ctx.lineWidth = 2;
    ctx.beginPath();
                  ...
                  <div className="bg-white rounded-xl shadow-xl p-5 border-l-4 border-purple-600">
                    ...
                    <div className="mb-4">
                      ...
                      <div className="grid grid-cols-2 gap-2 text-xs">
                        ...
                        {/* NOUVEAU : Réservoir à vessie - MASQUÉ pour relevage */}
                        {drawingData.installation_type !== 'submersible' && (
                          <label className={`flex items-center p-2 rounded cursor-pointer ${
